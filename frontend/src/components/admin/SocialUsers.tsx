...
const SocialUsers = () => {
  ...
  const handleSearch = () => {
    if (startDate && endDate) {
      toast.success(`Searching users from ${startDate} to ${endDate}`);
    } else {
  ...
  const handleDeleteRange = () => {
    if (startDate && endDate) {
      if (window.confirm(`Are you sure you want to delete users from ${startDate} to ${endDate}?`)) {
        toast.success(`Deleting users from ${startDate} to ${endDate}`);
      }
    } else {
      toast.error('Please select both start and end dates');
    }
  ...
  const confirmDelete = () => {
    if (selectedUser) {
      toast.success(`Deleting user: ${selectedUser.name}`);
  ...
  const confirmEdit = () => {
    if (editedUser) {
      toast.success(`Saving changes for user: ${editedUser.name}`);
  ...
  const confirmSendWhatsApp = () => {
    if (selectedUser && whatsAppMessage) {
      toast.success(`Sending message to ${selectedUser.name}: "${whatsAppMessage}"`);
