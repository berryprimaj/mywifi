                ...
                @foreach($profiles as $profile)
                    <div class="border rounded-lg p-4 flex flex-col">
                        ...
                        <div class="flex justify-end space-x-2 mt-4">
                            <button class="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-100">Edit</button>
                            <button class="px-3 py-1 border border-red-500 text-red-500 rounded-md text-sm hover:bg-red-50">Delete</button>
                        </div>
                    </div>
                @endforeach
            </div>
...
<script>
    ...
    mikrotikConfigForm.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('{{ route('admin.router_config.save') }}', {
                method: 'POST',
                headers: {
    ...
    testConnectionButton.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route('admin.router_config.test_connection') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Content-Type': 'application/json'
